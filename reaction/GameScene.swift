...
//            let resizew = SKAction.resize(toWidth: horixontalDegreeToPixel(sizeInDegree: 0.2), duration: 0.1)
            ...
            //fixButton.run(SKAction.sequence([waitactionm2,colorize,resizeh,resizew]))
            ...
            bigButton.run(SKAction.sequence([waitactionm1,actionMovem1,waitactionm,actionMovem]),completion: {
                self.first = true
...
//            let resizew = SKAction.resize(toWidth: horixontalDegreeToPixel(sizeInDegree: 0.2), duration: 0.1)
    ...
    func blockFinished()
    {
        ...
        print(str)
        filename = getDocumentsDirectory().appendingPathComponent("output_reaction.dipm")
        do {
        ...
        logfilename = getDocumentsDirectory().appendingPathComponent("log_reaction.dipm")
        do {
...
//                    fixButton.color = SKColor.yellow
                    pointlabel.text = "No answer"
                    resetTrial()
...
//            let hideaction3 = SKAction.hide()
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if blockStarted == false
        {
            ...
            finishlabel.text = "Tap to finish"
            resetTrial()
            blockStarted = true
...
//                {
                point = CACurrentMediaTime()-blocksStartTime - utime
                pointts = Int(1000*point)
                            //fixButton.color = SKColor.green
                            answers.append((trial,point,firstFlashInteral,secondFlashInteral,(trialEndTime - trialStartTime),firstWaitInterval))
...
//                fixButton.size.width = horixontalDegreeToPixel(sizeInDegree: 1)
//                fixButton.color = SKColor.red
                    print("l0l0llllllllllllllllllllllllllllll0l0l0lllllllllll")
                pointlabel.text = "Too early"
                answers.append((trial,20,firstFlashInteral,secondFlashInteral,(trialEndTime - trialStartTime),firstWaitInterval))
                resetTrial()
                }
                }
            }
